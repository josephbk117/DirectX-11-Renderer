...
Terrain::Terrain(Graphics& gfx, const std::string& heightMap, const TerrainInitInfo& initInfo) noexcept : terrainInitInfo(initInfo)
{
...

	{
		...
		{
			const float height = ((heightMapData[i].GetR() * (1.0f / 255.0f)) * 2.0f - 1.0f) * initInfo.heightScale;
	...
	AddBind(Sampler::Resolve(gfx, pixelSamplerSettings, samplerPipelinePixelInfo));

	SamplerSettings domainSamplerSettings
	{
	...
	std::vector<PipelineStageSlotInfo> samplerPipelineDomainInfo;
	samplerPipelineDomainInfo.push_back({ PipelineStage::VertexShader, 0 });
	samplerPipelineDomainInfo.push_back({ PipelineStage::DomainShader, 0 });
	AddBind(Sampler::Resolve(gfx, domainSamplerSettings, samplerPipelineDomainInfo));
...
void Terrain::ShowWindow(const char* windowName) noexcept
{
	...
	if (ImGui::Begin(windowName))
	{
		...
		ImGui::SliderFloat("Min Tessellation Distance", &detailInfo.minDistance, 100.0f, detailInfo.maxDistance);
		ImGui::SliderFloat("Max Tessellation Distance", &detailInfo.maxDistance, detailInfo.minDistance, 2500.0f);
